...
# 检查命令行参数
if len(sys.argv) < 2:
    print("用法: python budget_reallocation.py <数据文件路径> [总调配限额] [单渠道增幅上限]")
    print("示例: python budget_reallocation.py examples/analyzing-marketing-campaign/material/campaign_data_week1_china.csv 10000 0.15")
    sys.exit(1)
...
df = pd.read_csv(data_file, skiprows=1, names=column_names)
...
# 用户约束（可以通过命令行参数覆盖）
USER_REALLOCATION_LIMIT = float(sys.argv[2]) if len(sys.argv) > 2 else 10000.0  # 总调配限额，默认 ¥10,000
PER_CHANNEL_INCREASE_CAP = float(sys.argv[3]) if len(sys.argv) > 3 else 0.15    # 单渠道增幅上限，默认 15%
